import type { Metadata } from "next";
import { ApplyForm } from "@/components/forms/apply-form";
import { Card } from "@/components/ui/card";
import { Heading } from "@/components/ui/heading";
import { InnerPage } from "@/components/layout/inner-page";
import { Section } from "@/components/ui/section";

export const metadata: Metadata = {
  title: "Apply as a Volunteer",
  description:
    "Mentor a scholar, tutor weekly, or run a workshop. Tell us how you'd like to help and we'll match you to a scholar who needs it.",
};

export default function ApplyVolunteerPage() {
  return (
    <InnerPage>
      <Section as="header" variant="muted" className="py-12 md:py-16">
        <div className="mx-auto max-w-2xl">
          <Heading
            level={1}
            eyebrow="Get involved"
            description="Mentor a scholar, tutor weekly, or run a workshop. Tell us how you'd like to help and we'll match you to a scholar who needs it."
          >
            Apply as a Volunteer
          </Heading>
        </div>
      </Section>
      <Section className="py-10 md:py-12">
        <div className="mx-auto max-w-2xl">
          <Card className="p-5 md:p-8">
            <ApplyForm type="volunteer" />
          </Card>
        </div>
      </Section>
    </InnerPage>
  );
}
